...
function createLink() {
  ...
  var str=prompt("Enter link location (e.g. http://):", isA ? isA.href : "http:\/\/");
  
  if ((str!=null) && (str!="http://")) {
    if (Composition.document.selection.type=="None") {
      var sel=Composition.document.selection.createRange();
      sel.pasteHTML("<A HREF=../../student_new/mycourse/xmlpaper//%22%22+str+%22/%22>"+str+"</A> ");
      sel.select();
    }
...
//2004-03-04 HouGuangmin modify
function createImage(imgurl) 
{
	if (arguments.length==0)
	{ 
		window.open('img_upload.html','img_upload','width=481 height=190');
	}
	else 
	{ //those code had no use.	
  	if (!validateMode()) return;
  
  	var isA = getEl("A",Composition.document.selection.createRange().parentElement());
  	var str//=prompt("Enter image location (e.g. http://www.tongfangedu.com/gif/edu.gif):", isA ? isA.href : "http:\/\/");
  
  	if ((str!=null) && (str!="http://")) {
    if (Composition.document.selection.type=="None") {
      var sel=Composition.document.selection.createRange();
      sel.pasteHTML("<img src=../../student_new/mycourse/xmlpaper//%22%22+imgurl+%22/%22>");
      sel.select();
    }
...
function isRTextMode() {
  if (! bTextMode) return true;
  alert('现在的模式为文本模式,进行编辑操作,请先去掉\"文本\"的选择!');
  mytext.focus();
  return false;
}
function creatTable()
{
  ...
  var arr = showModalDialog("table.html", "", "dialogWidth:17em; dialogHeight:12em; status:0");
  if (arr != null){
  var tmp=arr.split("*");
  row=tmp[0];
  col=tmp[1];
  var string;
  string="<table style='WIDTH: "+tmp[2]+"px;' border="+tmp[3]+" bgcolor="+tmp[4]+" cellspacing='0' cellpadding='0'>";
  for(i=1;i<=row;i++){
  string=string+"<tr>";
  for(j=1;j<=col;j++){
  string=string+"<td>&nbsp;</td>";
