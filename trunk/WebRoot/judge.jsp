...
<% if (newscores != null) {
%>
alert("试卷经过盘卷后最后得分是<%=totalScore%>分");
...
<tr height="22">
  <td colspan="4" >&nbsp;<input type="button" class="bt" value="提交并关闭(H)"  accesskey="H" onclick="judgeEnd();"> <input type="button" class="bt" value="关闭(X)"  accesskey="C" onclick="window.close();"></td>
</tr>
</table>
  ...
  while (allanswer1.indexOf('\u0002') >= 0) {
    ...
    if (aa.type == "1" || aa.type == "2") {
      ...
      if (getInputValue("a"+allquestions[ind]) == "N") {
        if (!canModifyScore) {
          aa.innerHTML = "<br/>本题得分<input max=\""+max+"\"  name=\"j"+allquestions[ind]+"\"  type=\"text\" size=\"3\" maxlength=\"4\" disabled value=\""+score+"\">分，标准答案：";
        }
        else {
          aa.innerHTML = "<br/>本题得分<input max=\""+max+"\"  name=\"j"+allquestions[ind]+"\"  type=\"text\" size=\"3\" maxlength=\"4\" value=\""+score+"\">分，标准答案：";
        }
      }
      else {
        aa.innerHTML = "<br/>本题得分<input max=\""+max+"\"  name=\"j"+allquestions[ind]+"\"  type=\"text\" size=\"3\" maxlength=\"4\"  value=\""+score+"\">分，参考答案：";
      }
      var dd = 0;
      ...
      if (!canModifyScore) {
        aa.innerHTML = "<br/>本题得分<input max=\""+max+"\"  name=\"j"+allquestions[ind]+"\"  type=\"text\" size=\"3\" maxlength=\"4\" disabled value=\""+score+"\">分，标准答案："+(qra == "Y" ? "正确" : "错误");
      }
      else {
        aa.innerHTML = "<br/>本题得分<input max=\""+max+"\"  name=\"j"+allquestions[ind]+"\"  type=\"text\" size=\"3\" maxlength=\"4\" value=\""+score+"\">分，标准答案："+(qra == "Y" ? "正确" : "错误");
      }
    }
    else if (aa.type == "6") {
