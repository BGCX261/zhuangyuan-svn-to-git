...
function isDemoStudent(cid) {
  var result = false;
  for (var i = 0; i < demoStudents.length; i++) {
    if (demoStudents[i] == cid) {
      result = true; break;
    }
  }
  return result;
...
function setCourse(courseid, nowcs) {
  if (isDemoStudent(courseid)) {
    //alert("你要修改的是系统定义的演示学生，修改操作无效！"); return;
  }
  var aa = prompt("请输入新的关联课程，不同的课程以空格分隔！",nowcs);
  if (aa == null ) {
	  return;
  }
  location = "coursestudent.jsp?CID="+courseid+"&V="+httpAddrEncode(aa);
}
function doDel(cid) {
  if (isDemoStudent(cid)) {
    //alert("你要删除的是系统定义的演示学生，删除操作无效！"); return;
  }
  location="delstudent.jsp?CID="+cid;
}
function doAdd() {
	...
	if (document.form1.stuclass.value == "") {
		alert("部门能为空！");
		return;
	}
...
function modifyPassword(cid) {
  ...
  location = "modifyspass.jsp?CID="+cid+"&V="+httpAddrEncode(aa);
}
</script>
<style>
.rhinput {
...
<form name="form1" method="post" action="addstudent.jsp">
			  ...
			  <td  class="contentwindow">
				<table height="100%" width="100%" border="0" cellpadding="0" cellspacing="0" >
				  <tr  height="17" >
			        ...
			        <td colspan="2" class="contentwindowtitle">欢迎管理员进入本，你可以在这里添加学生、删除学生、修改学生。</td>
			      </tr>
				  <tr >
				    <td width="10"></td>
					<td width="100%">
					  <table height="100%" width="100%" border="0" cellpadding="0" cellspacing="0" leftmargin="10">
					    ...
					    <tr height="10">
						  <td  width="100%" class="content"  >&nbsp;&nbsp;&nbsp;&nbsp;以下是本系统的所有学生&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
for (int i = 0; i < csv_user.allRecords.length; i++) {
	if (csv_user.allRecords[i][4].equals("1")) continue;
  if (i % 2 == 0) cls = "orow";
  else cls = "erow";
...
<% } %>
					   ...
					   <tr height="20">
						  ...
						  部门<select class="rhinput" name="stuclass" id="stuclass" ><option value="">请选择</option><%= zy.util.CsvFile.getClassOptions() %></select>
							 <input type="button" class="bt" value="添加学生"  onclick="doAdd();">
                          </td>
					   </tr>
					    <tr height="15">
						  <td class="content" ></td>
